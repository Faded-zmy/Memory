...
async def _handle_connection(websocket, path):
    ...
    if path == '/api/v1/stream':
        ...
        async for message in websocket:
            ...
            for i in range(len(body['messages'][:-1])//2):
                history['internal'].append([body['messages'][2*i]['content'],body['messages'][2*i+1]['content']])
                history['visible'].append([body['messages'][2*i]['content'],body['messages'][2*i+1]['content']])
            print('ZMY-history-trans',history)
