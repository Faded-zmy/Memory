...
def build_parameters(body, chat=False):
    ...
    if chat:
        # character = body.get('character','Assistant')
        instruction_template = body.get('instruction_template')
        # print('ZMY-cha',character)
        # name1, name2, _, greeting, context, _  = load_character_memoized('None', 'User', shared.settings['name2'], instruct=False)#character
        name2 = 'ASSISTANT'
        name1 = 'USER'
        greeting = ''
        ...
        name1_instruct, name2_instruct,  context_instruct ,turn_template= '','','',''#zmy
