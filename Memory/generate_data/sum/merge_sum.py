...
def PostProcess(result):
    result = result.strip().replace('Summary: ', '')
    return result


def generate_prompt_reply(summary, log, api_id):
    ...
    res = {'org_summary': summary, 'merged_summary': result}
    return res
...
def work(start, end, total_summary, output_dir, split_id, api_id):
    log = logging.getLogger('mengying.zhou')
    log.setLevel('INFO')
    date = str(datetime.now().date())
    if not os.path.exists('/ai_jfs/mengying/data/sum/log/log1/' + date):
    ...
    res = []
    while right_end < end:
        num = random.choices([2,3,4,5,6,7,8], weights=[0.3,0.2,0.2,0.1,0.1,0.05,0.05])[0]
        # print('num',num)
        summary = total_summary[right_end:min(end,right_end+num)]
        right_end += num
        res_one = generate_prompt_reply(summary=summary, log=log, api_id=api_id)
        res.append(res_one)
    ...
    for root_path in folders:
        ...
        for file in os.listdir(root_path):
            if file.endswith('.json'):
                data = json.load(open(root_path+'/'+file, 'r'))['Dis']
                for d in data:
                    try:
                        total_summary.append(d['sum'])
                    except Exception as e:
                        print(e)
                        print(d)

        random.shuffle(total_summary)
        num_process = 20
        division = len(total_summary) // num_process
        ranges = [i*division for i in range(num_process)]
        processes = []
        output_dir = '/ai_jfs/mengying/data/sum/merge_sum/'+root_path.split('/')[-1]+'/'
        ...
        for i in range(num_process):
            ...
            if i == num_process - 1:
                start = ranges[i]
                end = len(total_summary)
                p = Process(target=work, args=(start, end, total_summary, output_dir, i, api_id))
            else:
                start = ranges[i]
                end = ranges[i+1]
                p = Process(target=work, args=(start, end, total_summary, output_dir, i, api_id))
